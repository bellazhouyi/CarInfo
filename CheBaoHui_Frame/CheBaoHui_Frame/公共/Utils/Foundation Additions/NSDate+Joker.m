...
+(NSString*)dateStringWithFormat:(NSString*)format timestamp:(NSString*)timestamp{
    ...
    long long secs = [timestamp longLongValue];
...
- (NSString *)stringMonthDayCompareToday
{
    NSString *str;
    NSInteger chaDay = [self daysBetweenCurrentDateAndDate];
    if (chaDay == 0) {
...
+ (NSString*)intervalSinceNowWithTimeStamp:(NSString*)timeStamp{
    ...
    NSTimeInterval late = [timeStamp doubleValue];
    ...
    }else if(timeInterval/3600 > 1 && timeInterval/86400 < 1) {//在1小时以上24小时以内
        
        timeString = [NSString stringWithFormat:@"%f", timeInterval/3600];
        timeString = [timeString substringToIndex:timeString.length-7];
        timeString = [NSString stringWithFormat:@"%@小时前", timeString];
        
        
    }else if(timeInterval/86400 > 1 && timeInterval/(86400*7) < 1){//1天以上一周(7天)以内
        
        timeString = [NSString stringWithFormat:@"%f", timeInterval/86400];
        timeString = [timeString substringToIndex:timeString.length-7];
        timeString=[NSString stringWithFormat:@"%@天前", timeString];
        
    }else{//一周以上
        
        return [NSDate dateStringWithFormat:@"yyyy-MM-dd" timestamp:timeStamp];
    }
 ...
 */
+(long long int)intervalWithBeginDate:(NSDate*)beginDate
                          endDate:(NSDate*)endDate{

    
    NSTimeInterval ms = [endDate timeIntervalSinceDate:beginDate];
