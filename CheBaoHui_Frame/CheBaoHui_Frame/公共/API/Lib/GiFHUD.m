...
#define Size            100
#define FadeInDuration    0.3
#define FadeOutDuration  1.5
#define GifSpeed        0.3
#define GifSpeedBiggerFrame 0.7
...
static GiFHUD *instanceBiggerFrame;
...
+(instancetype) instanceBigFrame{
    if (!instanceBiggerFrame) {
        instanceBiggerFrame = [[GiFHUD alloc] initForBiggerFrame];
    }
    return instanceBiggerFrame;
}

-(instancetype)initForBiggerFrame{
    if ((self = [super initWithFrame:CGRectMake(kUIScreenWidth * 0.1, kUIScreenHeight * 0.5, kUIScreenWidth * 0.7, kUIScreenHeight * 0.2)])) {
        ...
        self.imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, kUIScreenWidth * 0.9, kUIScreenHeight * 0.2)];
        [self addSubview:self.imageView];
...
#pragma mark -- 自己添加的显示文字
+ (void)showWithMessage:(NSString *)message{
    [self dismiss:^{
        [APPDELEGATE.window bringSubviewToFront:[self instanceBigFrame]];
        
        [[self instanceBigFrame] setLabelWithMessage:message];
        
        [[self instanceBigFrame] setShown:YES];
        [[self instanceBigFrame] fadeIn];
    }];
}


- (void) setLabelWithMessage:(NSString *)message{
    ...
    label.textColor = [UIColor colorWithRed:0 green:124 blue:195 alpha:1];
...
+ (void)setGifWithImagesByBiggerFrame:(NSArray *)images {
    [[[self instanceBigFrame] imageView] setAnimationImages:images];
    [[[self instanceBigFrame] imageView] setAnimationDuration:GifSpeedBiggerFrame];
}

+ (void)dismissBiggerFrame {
    if (![[self instanceBigFrame] shown])
        return;
    
    [[[self instanceBigFrame] overlay] removeFromSuperview];
    [[self instanceBigFrame] fadeOut];
    instanceBiggerFrame = nil;
}
